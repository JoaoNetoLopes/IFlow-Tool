...
# Method: GET
# Function: Renders home page. If the user is an authorized admin then the create election button is also rendered.
# request -> low
# return -> high
# Checks if variable is in authorized admins so is it "sanitized"?
...
# request -> high 
# return -> high
# Simply makes the connection to allow fenix login, no danger? Also no input given from user.
...
# request -> high 
# return -> high
# Since it requires login beforehand the user has no control over this. Also no input given from user
@login_required
def manageElections(request: LCHI = LCHI()):
    context = {'elections': Election.objects.filter(admin=request.user)} #type: HCLI()
...
# request -> high
# return -> high
# Again the username just asks for a json with the elections, no input given.
def getElections(request: LCHI = LCHI()):
    if request.method == 'GET':
        data = serializers.serialize('json',list(Election.objects.all())) #type: HCLI()
...
# User input in the form of a json with means everything derived from it will be low.(data)
@login_required
@csrf_exempt
def createElection(request: LCHI = LCHI()):
    ...
    def postCreateElection(request: LCHI = LCHI()):
        try:
            data = json.loads(request.body.decode('utf-8')) # type: Dict[str, LCHI]
            #data = Dict[str, LCHI]
            #Everything derived from data is user input so Low.
            #Everything derived from election is High.
...
# User input in the form of a json(trusteeData) which means the id/name/email will be points of entry.



@login_required
@csrf_exempt
def addTrustees(request: LCHI = LCHI(),election_id:LCHI = LCHI()):
    if request.method == 'POST':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
            ...
            data = json.loads(request.body.decode('utf-8')) # type: Dict[str, LCLI()]
            with transaction.atomic():
                for trusteeData in data['trusteeList']: # type: Dict[str, LCHI()]
                    t = Trustee()
                    # t is High therefore all the variables derivated from t in left are High.()
                    t.election = election 
...
# User input in the form of a json(trusteeData) that is suposed to remove a trustee but can be a point of entry.

@login_required
@csrf_exempt
def removeTrustees(request: LCHI = LCHI(), election_id: LCHI = LCHI()):
    if request.method == 'POST':
        try:
            election = Election.objects.get(id=election_id)#type: HCLI()
        except Election.DoesNotExist:
...
# Login required but other than that only the election_id is Low, but still shouldn't affect anything if the election_id is wrong since it is only a get.

@login_required
def getTrustees(request: LCHI = LCHI(), election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
            return HttpResponse(json.dumps({'error':'Election does not exist'}), content_type='application/json', status=404)
        data = serializers.serialize('json',list(Trustee.objects.all().filter(election=election))) #type: HCLI()
        return HttpResponse(data,content_type='application/json')
    else:
        return HttpResponseNotAllowed(['GET'])
...
# The problem isn't when creating a VotersFile but when populating it (when the user's input is provided.)

@login_required
def addVoters(request: LCHI = LCHI(),election_id: LCHI = LCHI()):

    def addVotersFile(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
        try:
    ...
    @csrf_exempt
    def addVotersJson(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
        try:
            data = json.loads(request.body.decode('utf-8')) # type: Dict[str, LCLI()]
            with transaction.atomic():
                for voterData in data['voterList']: # type: Dict[str, LCLI()]
                    voter = Voter()
                    voter.election = election 
...
# Even though it can only be done by admind, csrf_exmpt should guarantee it is not a target for a script that can delete more voters than it is suposed to.

@login_required
@csrf_exempt
def removeVoters(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'POST':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
        ...
        try:
            data = json.loads(request.body.decode('utf-8')) # type: Dict[str, LCLI()]
            for voterData in data['voterList']: # type: Dict[str, LCLI()]
                Voter.objects.get(identifier=voterData,election=election).delete() # .delete() is dangerous since it depends on the voterData.
            return HttpResponse(json.dumps({  
                'success':True
                }),content_type='application/json') 
...
# Since it is only a GET, the attacker has no power to affect the database(information).

@login_required
def getVoters(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
# Again receives input through reading the json associated with the request. which means everything that comes from the json is Low.
# Check the consequences of login_required, csrf_exempt.

@login_required
@csrf_exempt
def addQuestions(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'POST':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
        ...
        try:
            data = json.loads(request.body.decode('utf-8')) # type: Dict[str, LCHI()]
            with transaction.atomic():
                for questionData in data['questionList']: # type: Dict[str, LCHI()]
                    question = Question()
                    question.election = election 
                    question.question = questionData['question']
                    question.save()
                    for answerData in questionData['answers']: # type: Dict[str, LCHI()]
                        answer = Answer() 
...
# Same problem as the previous function everything that comes from a json that has no sanitization is in danger("data" in this case).

@login_required
@csrf_exempt
def removeQuestions(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'POST':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
# TODOOOOOOOOOOOOOOOOO check if questions are private
# No danger in this function since it is only a GET, only if the questions are private.


@login_required
def getQuestions(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
# Can only be done before the election ends and after the election starts. It sends a post request to the 
...
@login_required
def register(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
@login_required
@csrf_exempt
def cast(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    try:
        election = Election.objects.get(id=election_id) #type: HCLI()
    except Election.DoesNotExist:
        return HttpResponse(json.dumps({'error':'Election does not exist'}), content_type='application/json', status=404)
        ...
        try:
            ...
            try:
                ...
                for i,question in enumerate(questions["questionList"]):
                    ...
                    questionData = data['ballot'][question["id"]] # type: Dict[str, LCLI()]
                    totalAlpha = 1
                    totalAlpha = 1
                    totalBeta = 1
                    for j,answer in enumerate(question["answers"]):
                        if answer["id"] not in questionData["answers"]:
                            return HttpResponse(json.dumps({'error':'Ballot is not well formed'}), content_type='application/json', status=500)
                        answerData = questionData["answers"][answer["id"]] # type: Dict[str, LCLI()]
        ...
        if voter.publicCredential:
            ...
            if election.openCastTime!=None and election.closeCastTime!=None:
                if datetime.datetime.now().time() <= election.openCastTime or datetime.datetime.now().time() >= election.closeCastTime:
                    return render(request,"closedBallotBox.html",{'election':election})
            try:
                ballot = Ballot.objects.get(election=election,publicCredential=voter.publicCredential) # type: HCLI()
                return render(request,"closedBallotBox.html",{'election':election})
            except Ballot.DoesNotExist:
                cryptoParameters = json.loads(election.cryptoParameters.replace('\'','"'))
                questions = getQuestionsAux(election)
...
#
@login_required
def election(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist: # Still shouldn't have an impact since if the user accesses another election he can't be the admin therefore can't do anything.
...
#
@login_required
def manageTrustees(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
#
@login_required
def manageVoters(request: LCHI = LCHI() ,election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
#
@login_required
def manageQuestions(request: LCHI = LCHI() ,election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
@login_required
def trustee(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    ...
    def getMethodTrustee(election,t:HCHI()):
        if datetime.datetime.now() < election.startDate:
            ...
            cryptoParams = json.loads(election.cryptoParameters.replace('\'','"')) #type: HCLI()
            questions = getQuestionsAux(election)
...
# TODO
@login_required
def trusteeElectionList(request: LCLI = LCLI()):
    if request.method == 'GET':
...
#
@login_required
def addElectionPublicKey(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'POST':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
#
@login_required
def bulletinBoard(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
#TODO
@login_required
def auditBallot(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
#
@login_required  
def manageTally(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
#
@login_required
def addPaperVoters(request: LCHI = LCHI(),election_id: LCHI = LCHI()):

    def addPaperVotersFile(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
        try:
            csvfile = request.FILES['csv']
            reader = csv.reader(csvfile.read().decode('utf-8-sig').split('\n'),delimiter=";")
            with transaction.atomic():
...
@login_required
@csrf_exempt
def removePaperVoters(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'POST':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
#
@login_required
def submitPaperResults(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'POST':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
#
@login_required
def submitPartialDecryption(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'POST':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
#
@login_required
def publishResults(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'POST':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
#
@login_required
def deleteElection(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'POST':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
...
@login_required
def aggregateEncTally(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
        ...
        if datetime.datetime.now() >= election.endDate:
            ...
            for question in questions["questionList"]: #type: HCLI()
                result[question['id']] = {}
                for answer in question["answers"]:
...
def exportData(request: LCHI = LCHI(),election_id: LCHI = LCHI()):
    if request.method == 'GET':
        try:
            election = Election.objects.get(id=election_id) #type: HCLI()
        except Election.DoesNotExist:
